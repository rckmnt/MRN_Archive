...
public class FlowerController : MonoBehaviour {
    ...
    private void StoreExtremePetalPositions()
    {
        ...
        for (int i = 0; i < petals.Length; i++)
        {
            ...
            //petalPositionsClosed[i] = new Vector3(0f, 0f, 0f);
        ...
        petalScaleClosed = new Vector3(0.2f, 0.2f, 1f);
    ...
    private void ExpandPetals(bool isExpanding)
    {
        for (int i = 0; i < petals.Length; i++)
        {
            if (isExpanding)
            {
                //petals[i].SetRelativePosition(petalPositionsOpen[i], 1.2f, Easing.Custom, "0.34,0.89,0.71,-0.33");
                petals[i].SetRelativePosition(petalPositionsOpen[i], 1.2f, Easing.Linear);
                petals[i].transform.GetChild(0).gameObject.SetScale(petalScaleOpen, 1.2f);
            }
            else
            {
                petals[i].SetRelativePosition(petalPositionsClosed[i], 0.2f, Easing.Custom, "0.05,0.55,0.36,0.92");
